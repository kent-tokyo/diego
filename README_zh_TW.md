...
無特權 Active Directory 安全診斷代理，使用純 Rust 編寫。

---

**DIEGO** 是一個針對 Active Directory 環境的後滲透偵察和安全診斷代理。它僅使用標準域用戶憑證運行，不產生任何嘈雜的網路製品，並以單一靜態二進制檔案的形式交付。
...
- **純 Rust** — 無 .NET CLR、無 PowerShell、無 Python 解釋器。每個協議交互 — Kerberos ASN.1 框架、LDAP、RC4-HMAC — 都在純 Rust（RustCrypto）中實現。這消除了 EDR 產品最激進地監視的 ETW / AMSI / Script Block Logging 攻擊面。
...
| EDR 可偵測性 | **低** — 無 .NET/PS/Python | 高 — .NET 反射、AMSI | 中等 | 高 — AMSI / Script Block Logging | 高 — .NET、已知簽章 | 中等 |
...
- 在 LDAP 和 Kerberos 請求之間應用隨機化延遲，以避免均勻的時間簽名。
