...
- **纯 Rust** — 没有 .NET CLR、没有 PowerShell、没有 Python 解释器。每个协议交互 — Kerberos ASN.1 帧、LDAP、RC4-HMAC — 都以纯 Rust (RustCrypto) 实现。这消除了 EDR 产品最积极监控的 ETW / AMSI / Script Block Logging 攻击面。
...
| EDR 可检测性 | **低** — 无 .NET/PS/Python | 高 — .NET 反射、AMSI | 中等 | 高 — AMSI / Script Block Logging | 高 — .NET、已知签名 | 中等 |
...
- 在 LDAP 和 Kerberos 请求之间应用随机化的随机延迟，以避免统一的时序签名。
