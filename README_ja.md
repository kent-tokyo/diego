...
**DIEGO** は、防御者および認可された評価者が、標準ドメインユーザー権限から Active Directory の危険な設定を把握するための診断ツールです。読み取り専用の LDAP / Kerberos 診断により、構造化された証拠（JSON / Markdown / HTML / MCP）とベースライン比較を提供し、**ディレクトリへの書き込みや OS コマンド実行は行いません**。ノイズの多いネットワーク動作を生成せず、単一の静的バイナリとして配布されます。（攻撃フレームワークではありません。[Threat Model](docs/THREAT_MODEL.md) を参照。）
